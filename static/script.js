...
// --- Performance & Data Tracking Globals ---
let frameCount = 0;
let lastFPSTime = 0;
let currentFPS = 0;
...
class ParticleSystem {
    ...
    addParticle() { let index = this.nextAvailableIndex; if (!this.activeFlags[index]) { this.activeParticleCount = Math.min(this.activeParticleCount + 1, this.maxParticles); } this.activeFlags[index] = 1; this.nextAvailableIndex = (index + 1) % this.maxParticles; const currentTime = performance.now(); const canvasCssWidth = canvas.clientWidth; const canvasCssHeight = canvas.clientHeight; this.positions[index * 2] = Math.random() * canvasCssWidth; this.positions[index * 2 + 1] = Math.random() * canvasCssHeight; this.velocities[index * 2] = (Math.random() - 0.5) * DRIFT_SPEED_SCALE * canvasCssWidth; this.velocities[index * 2 + 1] = (-0.3 * DRIFT_SPEED_SCALE - Math.random() * 0.3 * DRIFT_SPEED_SCALE) * canvasCssHeight; this.sizes[index] = PARTICLE_BASE_SIZE * (0.8 + Math.random() * 0.4); this.baseAlphas[index] = 1.0; this.ages[index] = 0; this.birthTimes[index] = currentTime; this.colors[index * 3] = 0.8 + Math.random() * 0.2; this.colors[index * 3 + 1] = 0.8 + Math.random() * 0.2; this.colors[index * 3 + 2] = 0.9 + Math.random() * 0.1; }
...
function init() {
    if (!initWebGL()) { 
        ...
        if(starContainer) starContainer.innerHTML = "<p style='color:red; text-align:center;'>WebGL not supported or failed to initialize.</p>";
        return;
    }
...
eventSource.addEventListener(SSE_EVENT_NAME, function(event) {
    try {
        ...
        console.error("JS ERROR: Failed to parse or process block summary data:", e, "Data was:", event.data);
    }
});
